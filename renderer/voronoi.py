...
def voronoi_solve(texture, mask, device):
    ...
    indices = torch.arange(0, h*w).cuda(device).reshape(h, w)
    idx_map = -1 * torch.ones((h,w), dtype=torch.int64).cuda(device)
    ...
    # nonzero_count = torch.count_nonzero(voronoi_map)
    # print("#### non zero 1: ", nonzero_count)
    # print("#### total_weight 1 : ", voronoi_map.shape)
