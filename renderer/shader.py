...
class HardNChannelFlatShader(ShaderBase):
	...
	def __init__(
		...
		ones = ((1.0,)*channels,) # ((1.0, 1.0, 1.0, 1.0),)
		zeros = ((0.0,)*channels,)
		...
		if not materials or not materials.ambient_color.shape[-1] == channels:
			materials = Materials(
				...
				shininess=0.0,
			) # material has only ambient color(1,1,1,1)
	...
	def forward(self, fragments: Fragments, meshes: Meshes, **kwargs) -> torch.Tensor:
		...
		texels = meshes.sample_textures(fragments) #[10, 96, 96, 1, 4]
		lights = kwargs.get("lights", self.lights)
		materials = kwargs.get("materials", self.materials)
		blend_params = kwargs.get("blend_params", self.blend_params)
		# Cole, F. et al., “Differentiable Surface Rendering via Non-differentiable Sampling”.
		# [10, 96, 96, 1, 4]
		colors = flat_shading(
			meshes=meshes,
			fragments=fragments,
