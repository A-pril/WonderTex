...
parser.add_argument(
    "--object_path",
    type=str,
    default='d0c0522e9a6d4684a2744b06143bf9a5.glb',
    required=True,
...
def make_grid(imgs):
    grid_up = np.concatenate([imgs[0], imgs[1]], axis=1)
    grid_bm = np.concatenate([imgs[2], imgs[3]], axis=1)
    grid = np.concatenate([grid_up, grid_bm], axis=0)
    ...
    for obj in scene_meshes() if single_obj is None else [single_obj]:
        ...
        for coord in obj.bound_box:
            ...
            if not ignore_matrix:
                #TODO: get matrix world
                # print(f"obj matrix world is {obj.matrix_world}")
...
def normalize_scene():
    ...
    # TODO: print
    # offset0 = -(bbox_min + bbox_max) / 2
    # print("dxyz: ",dxyz, "dist: ", dist, "offset0: ", offset0)
    ...
    # print("offset: ", offset)
    for obj in scene_root_objects():
        obj.matrix_world.translation += offset # [-0.5 0.5]
...
def get_calibration_matrix_K_from_blender(mode='simple'):
    ...
    # scale:  1.0 width:  512.0 height:  512.0 
    # resolution_percentage:  100 resolution_x:  512 resolution_y:  512
...
VIEWS = ["_front", "_right", "_back",  "_left", "_front_right", "_back_right", "_back_left", "_front_left"] # "_top"]
if random.random() > 0.8:   
    EXTRA_VIEWS = ["_top_front_right", "_top_back_right", "_top_back_left", "_top_front_left",
                    "_bottom_front_right", "_bottom_back_right", "_bottom_back_left", "_bottom_front_left"]
else:
    EXTRA_VIEWS = []
TAGS = ["flat_view", "flat_view_var", "top_view", "bottom_view"]
...
def save_images(object_file: str) -> None:
    ...
    scale , offset = normalize_scene() # # [-0.5 0.5]
    ...
    camera_locations = [
        np.array([0,-radius,0]), # front
        np.array([radius,0,0]),  # right
        np.array([0,radius,0]),  # back
        np.array([-radius,0,0]), # left

        np.array([radius,-radius,0]) / np.sqrt(2.),   # front right
        np.array([radius,radius,0]) / np.sqrt(2.),    # back right
        np.array([-radius,radius,0]) / np.sqrt(2.),   # back left
        np.array([-radius,-radius,0]) / np.sqrt(2.),  # front left

        np.array([radius,-radius,radius]) / np.sqrt(3.),   # top front right
        np.array([radius,radius,radius]) / np.sqrt(3.),    # top back right
        np.array([-radius,radius,radius]) / np.sqrt(3.),   # top back left
        np.array([-radius,-radius,radius]) / np.sqrt(3.),  # top front left

        np.array([radius,-radius,-radius/2]) / np.sqrt(3.),   # bottom front right
        np.array([radius,radius,-radius/2]) / np.sqrt(3.),    # bottom back right
        np.array([-radius,radius,-radius/2]) / np.sqrt(3.),   # bottom back left
        np.array([-radius,-radius,-radius/2]) / np.sqrt(3.),  # bottom front left
    ...
    for j in range(len(VIEWS)):
        ...
        inv_depth_map_min = inv_depth_map * valid_mask + 100 * invalid_mask
        depth_max = np.max(inv_depth_map)
        depth_min = np.min(inv_depth_map_min)   
#         np.savetxt('dep.txt', depth_map[128:-128,128:-128], delimiter=' ', fmt='%.2f')
        depth_map = (inv_depth_map - depth_min) / (depth_max - depth_min).clip(0,1)
        depth_map[invalid_mask] = 0
        depth_map = (depth_map * 255).astype(np.uint8) # np.uint8((depth_map / 10) * 65535) # 65535

        normal_map = data['normals'][index]
        normal_map[invalid_mask] = [0.5, 0.5, 1]
        normal_map = normal_map*255

        color_map = data['colors'][index]
        color_map[invalid_mask] = ground_color # random backgrond color to enhance data
        ...
        rgbs.append(color_map)
        norms.append(normal_map)

        if index % 4 == 3:
            dep_grid = make_grid(deps)
            rgb_grid = make_grid(rgbs)
            norm_grid = make_grid(norms)
            tag_id = index // 4

            Image.fromarray(rgb_grid.astype(np.uint8)).save(
                '{}/{}/rgb_{}.jpg'.format(args.output_folder, object_uid, TAGS[tag_id]), quality=100)
            Image.fromarray(norm_grid.astype(np.uint8)).save(
                '{}/{}/normals_{}.jpg'.format(args.output_folder, object_uid, TAGS[tag_id]), quality=100)
            Image.fromarray(dep_grid.astype(np.uint8)).save(
                '{}/{}/depth_{}.jpg'.format(args.output_folder, object_uid, TAGS[tag_id]), quality=100)
