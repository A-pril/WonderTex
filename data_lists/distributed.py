...
parser.add_argument('--mode', type=str, default='render', choices=['render_ortho', 'render_persp'],
...
parser.add_argument('--objaverse_root', type=str, default='.objaverse/hf-objaverse-v1',
                    help='Path to a json file containing a list of 3D object files.')

parser.add_argument('--save_folder', type=str, default=None,
                    help='Path to a json file containing a list of 3D object files.')

parser.add_argument('--custom_blender_path', type=str, default=None,
                    help='blender path.')
...
def check_task_finish(render_dir):
    ...
    if os.path.exists(render_dir):
        fpath = os.path.join(render_dir, "rgb_flat_view.jpg")
        # print(fpath)
...
def worker(
    ...
    while True:
        ...
        if  'render' in args.mode:
            if check_task_finish(view_path):
                queue.task_done()
        ...
        # print("### item: ", item, "### gpu: ", gpu)
        if args.mode == 'render_ortho':
            command = (
                f" CUDA_VISIBLE_DEVICES={gpu} "
                f" blenderproc run --custom-blender-path {args.custom_blender_path} blenderProc_ortho.py"
                f" --object_path {item} "
        ...
        #     command = (
        #         f" CUDA_VISIBLE_DEVICES={gpu} "
        #         f" blenderproc run --blender-install-path {args.custom_blender_path} blenderProc_persp.py"
...
if __name__ == "__main__":
    ...
    for item in model_paths[args.start_i:args.end_i]:
        if os.path.exists(os.path.join(args.objaverse_root, item+".glb")):
        ...
        elif os.path.exists(os.path.join(args.objaverse_root, os.path.basename(item))):
